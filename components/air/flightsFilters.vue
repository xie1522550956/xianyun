<template>
  <div class="filters">
    <el-row type="flex" class="filters-top" justify="space-between" align="middle">
      <el-col :span="8">
        单程：
        {{flightsDatas.info.departCity }} - {{flightsDatas.info.destCity}}
        /
        {{flightsDatas.info.departDate}}
      </el-col>
      <el-col :span="4">
        <el-select size="mini" v-model="airport" placeholder="起飞机场" @change="runFilters">
          <el-option
            v-for="(item, index) in (flightsDatas.options.airport || [])"
            :key="index"
            :label="item"
            :value="item"
          ></el-option>
   ...
   <el-col :span="4">
        <el-select size="mini" v-model="flightTimes" placeholder="起飞时间" @change="runFilters">
          <el-option
            v-for="(item, index) in flightsDatas.options.flightTimes"
            :key="index"
      ...
      <el-col :span="4">
        <el-select size="mini" v-model="company" placeholder="航空公司" @change="runFilters">
          <el-option
            v-for="(item, index) in flightsDatas.options.company"
            :key="index"
            :label="item"
            :value="item"
          ></el-option>
...
export default {
  ...
  watch: {
      flightsData(){
          this.flightsDatas = {...this.flightsData}
      }
  },
  mounted () {
    //   this.flightsDatas = this.flightsData
